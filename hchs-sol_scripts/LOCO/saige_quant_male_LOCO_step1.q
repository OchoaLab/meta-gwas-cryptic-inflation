#!/bin/bash
#SBATCH --job-name=1s1_hchs-sol_male
#SBATCH --output=1s1_hchs-sol_male.out
...
for i in "${!traits[@]}"; do
  ...
  echo "Running SAIGE STEP1 male: $trait and $age_trait"
  time Rscript saige_step1_quant_sex_LOCO.R -s male -t "$trait" 
done
