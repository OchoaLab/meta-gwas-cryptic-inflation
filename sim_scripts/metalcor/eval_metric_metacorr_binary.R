...
# Produces eval table: eval_tables/{simulation}_1_20_metacorr_binary.txt
simulation="sim1_h08"
file_name = "1G_3000n_100causal_500000m"
library(tidyverse)
library(simtrait)
library(optparse)
...
for (rep_num in 1:20) {
  ...
  # Read sex-stratified SAIGE outputs (binary trait)
  # File naming differs between sim4 and sim1-3
  if (sim == "sim4") {
    df_male <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                       "/sex/saige_output_", file_name, "_male.txt"))
    df_female <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                         "/sex/saige_output_", file_name, "_female.txt"))
  } else {
    df_male <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                       "/sex/saige_output_", file_name, "_male_new.txt"))
    df_female <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                         "/sex/saige_output_", file_name, "_female_new.txt"))
  }
  ...
  study_female <- df_female %>% mutate(N = N_case + N_ctrl) %>% 
    dplyr::select(id = MarkerID, chr = CHR, pos = POS, beta = BETA, se = SE, n = N)
  study_male <- df_male %>% mutate(N = N_case + N_ctrl) %>% 
    dplyr::select(id = MarkerID, chr = CHR, pos = POS, beta = BETA, se = SE, n = N)
  ...
  r_file <- paste0(r_dir, "/", simulation, "_rep", rep_num, "_sex_binary", method_suffix, ".txt")
...
out_file <- paste0("./eval_tables/", simulation, "_1_20_metacorr_binary", method_suffix, ".txt")
