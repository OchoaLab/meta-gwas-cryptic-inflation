#!/usr/bin/env Rscript
# Produces eval table: eval_tables/{simulation}_1_20_metacorr_subpop_quant.txt
...
for (rep_num in 1:20) {
  ...
  # Read 3 subpop outputs (sim1 uses plink glm, sim2-4 use SAIGE)
  if (sim == "sim1") {
    df_S1 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/plink_output_1G_3000n_100causal_500000m_S1.trait.glm.linear.ADD"))
    df_S2 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/plink_output_1G_3000n_100causal_500000m_S2.trait.glm.linear.ADD"))
    df_S3 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/plink_output_1G_3000n_100causal_500000m_S3.trait.glm.linear.ADD"))
  } else {
    df_S1 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/saige_output_", file_name, "_S1_quant.txt"))
    df_S2 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/saige_output_", file_name, "_S2_quant.txt"))
    df_S3 <- read_table_check(paste0("./", simulation, '/rep', rep_num,
                                     "/subpop/saige_output_", file_name, "_S3_quant.txt"))
  }
  ...
  if (sim == "sim1") {
    study_S1 <- df_S1 %>% dplyr::select(id = ID, chr = CHROM, pos = POS, beta = BETA, se = SE, n = OBS_CT)
    study_S2 <- df_S2 %>% dplyr::select(id = ID, chr = CHROM, pos = POS, beta = BETA, se = SE, n = OBS_CT)
    study_S3 <- df_S3 %>% dplyr::select(id = ID, chr = CHROM, pos = POS, beta = BETA, se = SE, n = OBS_CT)
  } else {
    study_S1 <- df_S1 %>% dplyr::select(id = MarkerID, chr = CHR, pos = POS, beta = BETA, se = SE, n = N)
    study_S2 <- df_S2 %>% dplyr::select(id = MarkerID, chr = CHR, pos = POS, beta = BETA, se = SE, n = N)
    study_S3 <- df_S3 %>% dplyr::select(id = MarkerID, chr = CHR, pos = POS, beta = BETA, se = SE, n = N)
  }
  ...
  r_file <- paste0(r_dir, "/", simulation, "_rep", rep_num, "_subpop_quant", method_suffix, ".txt")
...
out_file <- paste0("./eval_tables/", simulation, "_1_20_metacorr_subpop_quant", method_suffix, ".txt")
