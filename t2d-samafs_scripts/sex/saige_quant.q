...
#SBATCH --job-name=samafs_saige_male_female
#SBATCH --output=samafs_quant_sex.out
#SBATCH --mem=100G
...
time Rscript saige_step1_quant.R -s male -t "$trait" -a "$age_trait"
time Rscript saige_step2_quant.R -s male -t "$trait" -a "$age_trait"

# Run R scripts for female
echo "Running for female: $trait and $age_trait"
time Rscript saige_step1_quant.R -s female -t "$trait" -a "$age_trait"
time Rscript saige_step2_quant.R -s female -t "$trait" -a "$age_trait"
done
