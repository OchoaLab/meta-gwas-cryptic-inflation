...
if (analysis != 'all') {
  if (analysis == "sex") {
    ...
    GMMATmodelFile = paste0(dir, simulation, '/rep', rep_num, '/sex/saige_quant_', filename, ".rda") 
    varianceRatioFile = paste0(dir, simulation, '/rep', rep_num, '/sex/saige_quant_', filename,".varianceRatio.txt") 
    SAIGEOutputFile = paste0(dir, simulation, '/rep', rep_num, '/sex/saige_output_', filename, '_quant.txt') 
    
  } else { # subpop
    plinkFile = paste0(dir, simulation, '/rep', rep_num, "/subpop/", filename)
    GMMATmodelFile = paste0(dir, simulation, '/rep', rep_num, '/subpop/saige_quant_', filename, ".rda") 
    varianceRatioFile = paste0(dir, simulation, '/rep', rep_num, '/subpop/saige_quant_', filename,".varianceRatio.txt") 
    SAIGEOutputFile = paste0(dir, simulation, '/rep', rep_num, '/subpop/saige_output_', filename, '_quant.txt') 
  }
  # main analysis
} else {
    plinkFile = paste0(dir, simulation, '/rep', rep_num, "/", filename)
    GMMATmodelFile = paste0(dir, simulation, '/rep', rep_num, '/saige_quant.rda') 
    varianceRatioFile = paste0(dir, simulation, '/rep', rep_num, "/saige_quant.varianceRatio.txt") 
    SAIGEOutputFile = paste0(dir, simulation, '/rep', rep_num, "/saige_output_quant.txt") 
}
