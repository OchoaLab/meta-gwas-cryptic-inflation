...
for (x in 1:subpop){
  ...
  new_fam = fam %>% separate(id, c("group", "id"), sep = "-") %>% 
    ...
    mutate(id = as.numeric(id) + ifelse(x == 1, 0, 1000 * (x - 1)),
           pat = ifelse(is.na(pat) == TRUE, NA, as.numeric(pat) + 1000* (x - 1)),
           mat = ifelse(is.na(mat) == TRUE, NA, as.numeric(mat) + 1000* (x - 1))) %>% 
