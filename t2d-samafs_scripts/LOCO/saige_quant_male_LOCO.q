#!/bin/bash
#SBATCH --job-name=samafs_quant_male
#SBATCH --output=samafs_quant_male-chr%a.out
#SBATCH --mem=100G
#SBATCH --array=1-22
...
#time Rscript saige_step1_quant_sex_LOCO.R -s male -t "$trait" -a "$age_trait" 
time Rscript saige_step2_quant_sex_LOCO.R -s male -t "$trait" -a "$age_trait" -c "$chr_num"
done