#!/bin/bash
#SBATCH --job-name=1s2_hchs-sol_male
#SBATCH --output=1s2_hchs-sol_male-chr%a.out
...
for i in "${!traits[@]}"; do
  ...
  echo "Running SAIGE STEP2 male: $trait and $age_trait"
  time Rscript saige_step2_quant_sex_LOCO.R -s male -t "$trait" -c "$chr_num"
done
