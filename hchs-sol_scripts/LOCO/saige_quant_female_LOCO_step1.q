#!/bin/bash
#SBATCH --job-name=11s1_hchs-sol_female
#SBATCH --output=11s1_hchs-sol_female.out
...
for i in "${!traits[@]}"; do
  ...
  echo "Running SAIGE STEP1 female: $trait and $age_trait"
  time Rscript saige_step1_quant_sex_LOCO.R -s female -t "$trait" 
done
